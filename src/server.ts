...
import { 
  ...
  Smile, Search, ArrowLeft, Check, CheckCheck
} from 'lucide-react'
...
const ChatInterface = ({ currentUser, targetUser, onClose }: ChatInterfaceProps) => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      text: `Hi ${currentUser.firstName}! I saw you're interested in language exchange. I'm currently studying Chinese at ${targetUser.university}.`,
  ...
  const simulateReply = () => {
    ...
    setTimeout(() => {
      ...
      const newMessage: Message = {
        ...
        senderId: targetUser.id,
        text: "That sounds great! When are you free for a quick practice session?",
        timestamp: new Date(),
        status: 'sent'
      }
  ...
  // 1. Active Call Overlay (The "Very Advanced" Part)
  if (callState !== 'idle') {
    return (
  ...
  return (
    <div className="flex flex-col h-[85vh] bg-[#0a0f1c] border border-white/10 rounded-3xl overflow-hidden shadow-2xl relative">
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-[#050816]/80 backdrop-blur-md border-b border-white/5 z-10">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="p-2.5 rounded-full bg-white/5 hover:bg-cyan-500/20 hover:text-cyan-400 text-gray-400 transition-all"
          >
            <Phone className="w-5 h-5" />
          </button>
          <button 
            onClick={() => startCall('video')}
            className="p-2.5 rounded-full bg-white/5 hover:bg-purple-500/20 hover:text-purple-400 text-gray-400 transition-all"
          >
      ...
      <div className="p-4 bg-[#050816] border-t border-white/5">
        ...
        {showMediaPicker && (
          <div className="mb-4 p-4 bg-white/5 rounded-xl border border-white/10 flex items-center justify-between">
            <span className="text-sm text-gray-300">Select Photo or Video</span>
        ...
        <div className="flex items-end gap-3 bg-white/5 p-2 rounded-3xl border border-white/10 focus-within:border-cyan-500/50 transition-colors">
          <button 
            ...
            className="p-3 rounded-full text-gray-400 hover:text-cyan-400 hover:bg-white/5 transition-all"
          >
